...
export function createMcpServer(manager: LobbyManager, astrix?: { state: AstrixWorldState; bus: AstrixGameCommandBus }): McpServer {
  ...
  if (astrixTools) {
    ...
    server.registerTool("harvest", { description: "Harvest a mature ASTrix crop for food.", inputSchema: z.object({ crop_id: z.string() }) }, async (args) => text(await astrixTools.callTool("harvest", args)));
